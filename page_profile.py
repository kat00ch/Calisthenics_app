...
def load_profile_page(parent, user_data):
    ...
    def select_exercise(exo):
        path_utilisateur = f"data/utilisateurs/{user_data['nom']}.json"
        ...
        data_niveaux = data.get("niveaux", [])
        niveau_atteint = user_data.get("niveau", 0)

        # Sélection des niveaux débloqués
        if user_data["points_totaux"] >= data_niveaux[niveau_atteint].points_requis:
            user_data["niveau"] = niveau_atteint + 1
            label_niveaux.configure(text=f"Niveau: {user_data['niveau']}")
        
        progress_bar.set(min(user_data["points_totaux"] / data_niveaux[niveau_atteint].points_requis, 1))

        if not path_utilisateur.endswith(".json"):
            return
        try:
    ...
    ctk.CTkButton(frame_profil, text="Activer Malus",
                  command=lambda: show_malus_modal(frame_profil, user_data, data, label_points, progress_bar)).pack(side="bottom", pady=10)

def show_malus_modal(parent, user_data, data, label_points, progress_bar):
